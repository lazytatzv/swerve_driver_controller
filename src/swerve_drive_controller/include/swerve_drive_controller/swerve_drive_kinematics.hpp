...
public:
  ...
  /**
   * @brief Optimize wheel commands to minimize steering rotation while respecting angle limits.
   * If a wheel needs to rotate more than 90 degrees or violates physical angle limits,
   * flip the wheel velocity and adjust the steering angle by 180 degrees instead.
   * @param wheel_commands The computed wheel commands.
   * @param current_steering_angles Array of current steering angles (radians).
   * @param min_steering_angle Minimum allowed steering angle in radians (default: -pi).
   * @param max_steering_angle Maximum allowed steering angle in radians (default: pi).
   * @return Optimized wheel commands with minimal steering rotation within allowable limits.
   */
